...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.98\linewidth]{figures_9_20260922/fig01_system_architecture_ai.png}
  \bicaption{光栅扭矩测量数字仿真平台总体架构}{Overall architecture of the optical grating torque measurement simulation platform}
  \label{fig:arch}
\end{figure}

图\ref{fig:arch}给出了机械、光学、信号、算法和通信模块之间的接口关系；其中示意图用于说明模型分层，不作为实物结构或通信实测证据。
...
光栅副的结构示意如图\ref{fig:structure}所示。图中标注用于对应模型中的扭轴、主光栅、指示光栅和五路检测位置，具体尺寸仍以表\ref{tab:param}的仿真参数为准。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\linewidth]{figures_9_20260922/fig02_mechanical_optical_structure_ai.png}
  \bicaption{光栅扭矩测量机械—光学结构示意}{Schematic of the mechanical--optical structure for grating torque measurement}
  \label{fig:structure}
\end{figure}
...
图\ref{fig:signal_chain}进一步概括了从扭矩输入到ADC采样的信号链及误差注入位置。该图是模型流程示意，Stage 14的算法比较仍直接使用模拟四通道信号。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.98\linewidth]{figures_9_20260922/fig03_signal_error_model_ai.png}
  \bicaption{四相信号生成与误差注入链路}{Four-phase signal generation and error-injection chain}
...
图\ref{fig:protocol}展示了从估计结果、评价指标到协议帧和CRC校验的接口关系。图中“协议预览”不等于物理串口实测通信。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.98\linewidth]{figures_9_20260922/fig05_protocol_evidence_ai.png}
  \bicaption{结果评价与11字节协议帧接口示意}{Interface between result evaluation and the 11-byte protocol frame}
  \label{fig:protocol}
\end{figure}
...
图\ref{fig:algorithm_flow}给出了五种算法从输入信号、异常标记到位移和扭矩输出的比较流程。图中“12 bit ADC”表示可选接口，具体性能统计以模拟通道为输入。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.98\linewidth]{figures_9_20260922/fig04_algorithm_flow_ai.png}
  \bicaption{多算法解调与扭矩换算流程}{Multi-algorithm demodulation and torque-conversion flow}
...
各算法在14类工况下的RMSE热图如图\ref{fig:rmse_all}所示；该图直接对应Stage 14的工况—算法矩阵。全工况性能聚合统计结果如表\ref{tab:method_summary}和图\ref{fig:method_summary}所示。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.98\linewidth]{figures_9_20260922/fig06_rmse_heatmap_python.png}
  \bicaption{14类工况下五种解调算法的位移RMSE热图}{Displacement RMSE heatmap for five demodulation algorithms across 14 cases}
  \label{fig:rmse_all}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.98\linewidth]{figures_9_20260922/fig07_method_summary_python.png}
  \bicaption{五种解调算法的误差统计与归一化鲁棒性}{Error statistics and normalized robustness of five demodulation algorithms}
...
图\ref{fig:rmse_all}和图\ref{fig:method_summary}分别给出逐工况RMSE分布与聚合统计；两图替换原有Stage 14 PDF图件，并保留同一CSV数据来源。对于反向扭矩扫掠，结果只描述当前采样窗口和unwrap实现，不外推为所有动态反向工况的普适结论。

\subsection{鲁棒性评分与抗噪鲁棒性评价}
为量化评价算法在扰动下的品质衰减，定义归一化鲁棒性评分为$S_{\mathrm{rob}} = 1 / [1 + \mathrm{RMSE} / (P / 32)]$。图\ref{fig:method_summary}中的右侧面板给出了各算法的聚合评分。Patent 1/32在当前14个离线工况中的均值为0.638；该分数依赖预先选定的$P/32$归一化尺度，仅用于本组算法相对比较。
...
针对$-20~^\circ\mathrm{C}\sim +80~^\circ\mathrm{C}$（步长$20~^\circ\mathrm{C}$）范围，平台开展温度参数敏感性仿真。灵敏度变化和补偿前后扭矩误差如图\ref{fig:temp_sens}所示。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.98\linewidth]{figures_9_20260922/fig08_temperature_compensation_python.png}
  \bicaption{温度引起的灵敏度变化及模型逆变换误差}{Temperature-induced sensitivity variation and model-inversion error}
  \label{fig:temp_sens}
\end{figure}

如图\ref{fig:temp_sens}所示，温度从$20~^\circ\mathrm{C}$升至$+80~^\circ\mathrm{C}$时，完整相位链的灵敏度偏差达到$+1.83\%$；降至$-20~^\circ\mathrm{C}$时为$-1.19\%$。右侧面板显示同一模型逆变换后的残差为数值误差量级，不能解释为实物温漂测量结果。

\subsection{温度参数逆变换前后精度对比}
采用式\eqref{eq:temp_comp}的同模型逆变换进行离线补偿。数字孪生软件的运行态界面示例如图\ref{fig:software_gui}所示；全温区详细误差数值记录如表\ref{tab:temp_summary}所示，误差曲线数据已汇总在图\ref{fig:temp_sens}的右侧面板。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.98\linewidth]{figures_9_20260922/fig09_software_gui_user_runtime.png}
\bicaption{数字孪生软件运行态界面截图}{User-provided runtime screenshot of the digital-twin software}
...
由图\ref{fig:temp_sens}与表\ref{tab:temp_summary}可知：
